<!-- Datatables -->
<script src="<?= SERVER_NAME . "/assets/vendors/datatables/js/dataTables.min.js" ?>"></script>
<script src="<?= SERVER_NAME . "/assets/vendors/datatables/js/dataTables.bootstrap5.min.js" ?>"></script>

<!-- responsive data tables -->
<script src="<?= SERVER_NAME . "/assets/vendors/datatables/js/dataTables.responsive.min.js" ?>"></script>
<script src="<?= SERVER_NAME . "/assets/vendors/datatables/js/responsive.bootstrap5.min.js" ?>"></script>

<script src="<?= SERVER_NAME . "/custom-assets/js/components.js" ?>"></script>
<script src="<?= SERVER_NAME . "/assets/js/app.js" ?>"></script>
